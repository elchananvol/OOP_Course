...
public class BrickerGameMananger extends GameManager {
    ...
    private final danogl.util.Counter livesCounter =new danogl.util.Counter();
...
//    BrickerGameManager.NUM_OF_LIVES = 4
    ...
    public void initializeGame(ImageReader imageReader, SoundReader soundReader, UserInputListener inputListener, WindowController windowController) {
        ...
        this.ball = new Ball(Vector2.ZERO, new Vector2(50, 50), ball_image, sound_of_ball);
        set_ball_at_center();
        ...
        ImageRenderable liveImage = imageReader.readImage("assets/heart.png", true);
        GraphicLifeCounter graphicLife = new GraphicLifeCounter(new Vector2(BORDER_WIDTH+SIDE_BAR,windowDimension.y()-LIVES_FONT_SIZE - BORDER_WIDTH), new Vector2(LIVES_FONT_SIZE, LIVES_FONT_SIZE), livesCounter, liveImage,
                this.gameObjects(), NUM_OF_LIVES);
        NumericLifeCounter numericLifeCounter = new NumericLifeCounter(livesCounter,new Vector2(BORDER_WIDTH+SIDE_BAR,windowDimension.y()-LIVES_FONT_SIZE*2 - BORDER_WIDTH),new Vector2(LIVES_FONT_SIZE, LIVES_FONT_SIZE),this.gameObjects());


    }
    private void set_ball_at_center(){
    ...
    private void createBricks(ImageReader imageReader, Vector2 windowDimension) {
        ...
        float brick_width = (windowDimension.x() - SIDE_BAR - BORDER_WIDTH * 2 - SPACE_BETWEEN_BRICKS * (BRICK_COLS - 1)) / BRICK_COLS;
        ...
        for (int row = 0; row < BRICK_ROWS; row++) {
            for (int col = 0; col < BRICK_COLS; col++) {
                Vector2 location = new Vector2(BORDER_WIDTH + SIDE_BAR + col * brick_width + SPACE_BETWEEN_BRICKS * col, BORDER_WIDTH + row * BRICK_HEIGHT);
                GameObject brick = new Brick(location, brickDimension, brickImage, collisionStrategy, bricksCounter);
    ...
    public static void main(String[] args) {
        BrickerGameMananger game = new BrickerGameMananger("Brick", new Vector2(700, 700));
        game.run();
